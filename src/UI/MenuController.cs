...
public class MenuController
{
    ...
    public async Task RunAsync()
    {
        ...
        try
        {
            ...
            // Only warn about missing winmm.dll if SlotWeave was installed
            // (fresh game installs don't have it — that's expected)
            if (_gameDir != null
                && Directory.Exists(Path.Combine(_gameDir, "SlotWeave"))
                && !File.Exists(Path.Combine(_gameDir, "winmm.dll")))
